...
if __name__=="__main__":
    ...
    for i in range(0, np.shape(data1_X_train)[1]):
        ...
        error_rate_1[i] = sum(
            DT1.fit(ica.fit_transform(data1_X_train), data1_y_train).predict(ica.fit_transform(data1_X_train)) == data1_y_train) * 1.0
        print i + 1
    ...
    for i in range(0, np.shape(data2_X_train)[1]):
        ...
        error_rate_2[i] = sum(
            DT2.fit(ica.fit_transform(data2_X_train), data2_y_train).predict(ica.fit_transform(data2_X_train)) == data2_y_train) * 1.0
