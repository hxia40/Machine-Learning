...
class XQPlusLearningTableNC:
    ...
    def learn(self, s, a, r, s_, alpha):
        ...
        if s_ == s:
            q_target = r  # next state is terminal
            self.x_table.loc[s, a] = 0
            # print "====got to end====", s, a, q_target
        else:
            # print "====got to forward move====", s, a
            q_target = r + self.gamma * self.q_table.loc[s_, :].max()  # next state is not terminal
            self.x_table.loc[s, a] = max(self.x_table.loc[s, a] - 1, 0)
